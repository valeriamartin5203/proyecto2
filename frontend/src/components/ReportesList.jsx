    ...
    <div>
              ...
              <div className="mt-3 pt-2 border-top comments-section">
                <div className="mb-3" style={{ maxHeight: '300px', overflowY: 'auto' }}>
                  {cargandoComentarios[reporte.id] ? (
                    <div className="text-center py-3"><Spinner animation="border" size="sm" /><p className="mt-2 small">Cargando comentarios...</p></div>
                    ...
                    comentarios[reporte.id]?.map((com) => (
                      <div key={com.id} className="comentario-item">
                        <div className="comentario-avatar">{com.usuario?.charAt(0).toUpperCase() || 'U'}</div>
                        <div className="comentario-content">
                          <div className="comentario-header">
                            <span className="comentario-usuario">{com.usuario}</span>
                            <small className="comentario-fecha">{formatDate(com.fecha)}</small>
                          </div>
                          <p className="comentario-texto">{com.texto}</p>
                          <div className="comentario-actions">
                            <button className="comentario-action" onClick={() => handleLikeComentario(com.id)}>
                              {com.likes > 0 ? `❤️ ${com.likes}` : '❤️ Me gusta'}
                            </button>
                            <button className="comentario-action" onClick={() => handleToggleReply(com.id)}>
                          ...
                          {respuestas[com.id]?.length > 0 && (
                            <div className="mt-2">
                              {respuestas[com.id].map((res) => (
                                <div key={res.id} className="respuesta-item d-flex mt-2">
                                  <div className="comentario-avatar" style={{ width: '28px', height: '28px', fontSize: '11px' }}>{res.usuario?.charAt(0).toUpperCase() || 'U'}</div>
                                  <div className="comentario-content" style={{ backgroundColor: '#e8e8e8' }}>
                                    <div className="comentario-header">
                                      <span className="comentario-usuario">{res.usuario}</span>
                                      <small className="comentario-fecha">{formatDate(res.fecha)}</small>
                                    </div>
                                    <p className="comentario-texto">{res.texto}</p>
                                    <div className="comentario-actions">
                                      <button className="comentario-action" onClick={() => handleLikeRespuesta(res.id)}>
                          ...
                          {showReplyInput[com.id] && (
                            <div className="mt-2 comentario-input-group">
                              <input
                                type="text"
                                className="comentario-input"
                                placeholder="Escribe una respuesta..."
                                value={nuevaRespuesta[com.id] || ''}
                                onChange={(e) => setNuevaRespuesta(prev => ({ ...prev, [com.id]: e.target.value }))}
                                onKeyPress={(e) => e.key === 'Enter' && handleAddRespuesta(com.id)}
                              />
                              <button className="comentario-send" onClick={() => handleAddRespuesta(com.id)} disabled={enviandoRespuesta[com.id] || !nuevaRespuesta[com.id]?.trim()}>
                ...
                <div className="comentario-input-group">
                  ...
                  />
                  <button className="comentario-send" onClick={() => handleAddComentario(reporte.id)} disabled={enviandoComentario[reporte.id] || !nuevoComentario[reporte.id]?.trim() || !usuarioActual}>
            ...
            <Modal show={showShare[reporte.id]} onHide={() => handleToggleShare(reporte.id)} centered>
              ...
              <Modal.Body className="pt-0">
                ...
                <div className="d-flex gap-3 mb-4 justify-content-center">
                  <button className="btn-share btn-share-primary" onClick={() => handleShareToFacebook(reporte.id)}><Facebook size={24} /></button>
                  <button className="btn-share btn-share-info" onClick={() => handleShareToTwitter(reporte.id, reporte)}><Twitter size={24} /></button>
                  <button className="btn-share btn-share-secondary" onClick={() => handleShareToEmail(reporte.id, reporte)}><Envelope size={24} /></button>
                </div>
                <div className="border rounded-3 p-2 bg-light">
                  <div className="d-flex align-items-center gap-2">
                    <Link45deg className="text-muted" />
                    <Form.Control type="text" value={`${window.location.origin}/reporte/${reporte.id}`} readOnly size="sm" className="bg-light border-0 small" />
                    <Button variant={copied[reporte.id] ? 'success' : 'outline-secondary'} size="sm" onClick={() => handleCopyLink(reporte.id)}>{copied[reporte.id] ? <Check2 size={14} /> : 'Copiar'}</Button>
                  </div>
                </div>
              </Modal.Body>
            </Modal>
      ...
      <Modal show={imagenSeleccionada && showImageModal[imagenSeleccionada?.id]} onHide={() => { if (imagenSeleccionada) { setShowImageModal(prev => ({ ...prev, [imagenSeleccionada.id]: false })); setTimeout(() => setImagenSeleccionada(null), 300); } }} centered size="lg" className="image-modal">
        <Modal.Header closeButton className="border-0 pb-0">
          <Modal.Title>
            <div className="d-flex align-items-center">
              <div className="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center me-2" style={{ width: '32px', height: '32px' }}>{imagenSeleccionada?.usuario?.charAt(0).toUpperCase() || 'U'}</div>
        ...
        <Modal.Body className="pt-0 text-center">
          {imagenSeleccionada && (
            <div>
              <img src={imagenSeleccionada.imagen} alt="Evidencia" className="modal-imagen" onError={(e) => e.target.src = 'https://via.placeholder.com/800x600?text=Imagen+no+disponible'} />
